...
const Contact = () => {
  ...
  return (
    <div id='contact' className='dark:bg-none w-full px-[5%] sm:px-[8%] lg:px-[12%] py-10 scroll-mt-20 bg-[url("/footer-bg-color.png")] bg-no-repeat bg-center bg-[length:90%_auto]'>
      <h4 className='text-center mb-2 text-lg font-Ovo'>Conecta conmigo</h4>
      <h2 className='text-center text-3xl sm:text-4xl lg:text-5xl font-Ovo'>Contáctame</h2>
      ...
      <form onSubmit={onSubmit} className='max-w-2xl mx-auto px-4'>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mt-10 mb-8'>
            ...
            rounded-md bg-white dark:border-white/90 text-sm sm:text-base' name='nombre'/>
            <input type="email" placeholder='Ingresa tu correo' required
            className='flex-1 p-3 outline-none border-[0.5px] border-gray-400 dark:text-gray-800
            rounded-md bg-white dark:border-white/90 text-sm sm:text-base' name='correo'/>
        </div>
        ...
        rounded-md bg-white mb-6 dark:border-white/90 text-sm sm:text-base' name='mensaje'></textarea>

        <button type='submit'
        className='py-3 px-8 w-max flex items-center justify-between gap-2 bg-black/80 text-white rounded-full mx-auto hover:bg-black duration:500
        dark:bg-transparent dark:border-[0.5px] text-sm sm:text-base'
        >Enviar ahora <Image src={assets.right_arrow_white} alt='' className='w-4'/></button>
